...
void MapSearchGreedy::solve(Timer &timer, long long iteration) {
  sort(this->graph->getNodes().begin(),
       this->graph->getNodes().end()); // sort graph node list
  deque<Node> start_node_list =
      this->getStartIndexList(); // start node index list for iterative solving
  ...
  for (int i = 0; i < min(iteration, (long long)this->graph->getNodes().size());
       i++) {
    ...
    while (!copyMap.empty()) {
      ...
      if (differ_x == MAX && min_differ_y == MAX) {
        ...
        long long min_squared_distance =
            min_differ_y * min_differ_y; // if no nodes in same x then MAX*MAX
        long long curr_squared_distance;

        // search up
        temp_x_it = xit;
        temp_x_it++;
        while (temp_x_it != copyMap.end() &&
               (temp_x_it->first - xit->first) *
                       (temp_x_it->first - xit->first) <
          ...
          if (temp_y_it == temp_x_it->second.end()) {
            ...
            temp_y_it = temp_x_it->second.find(
                curr_node.y); // insert temp Node and find iterator
            map<int, Node>::iterator y_plus_it, y_minus_it;
            y_plus_it = temp_y_it;
            y_plus_it++;
            y_minus_it = temp_y_it;
            y_minus_it--;
            temp_x_it->second.erase(
                temp_y_it); // find up down iterator and erase temp node
            ...
            if (y_plus_it == temp_x_it->second.end()) {
              ...
              if (y_plus_it->first - curr_node.y >
                  curr_node.y - y_minus_it->first) {
                temp_y_it = y_minus_it;
              } else {
                temp_y_it = y_plus_it;
              }
            ...
            curr_squared_distance =
                this->eucl->getSquaredDistance(curr_node, temp_y_it->second);
            if (curr_squared_distance < min_squared_distance) {
              min_squared_distance = curr_squared_distance;
              nearest_node = temp_y_it;
            }
          }
          // if value exist then measure distance and update min_sqr,
          // nearest_node
          else {
            curr_squared_distance =
                this->eucl->getSquaredDistance(curr_node, temp_y_it->second);
        ...
        while (temp_x_it != copyMap.end() &&
          ...
          if (temp_y_it == temp_x_it->second.end()) {
            Node tnode;
            temp_x_it->second[curr_node.y] = tnode;
            temp_y_it = temp_x_it->second.find(curr_node.y); //´Ù½Ã Ã£°í
            map<int, Node>::iterator y_plus_it, y_minus_it;
            y_plus_it = temp_y_it;
            y_plus_it++;
            y_minus_it = temp_y_it;
            y_minus_it--;
            temp_x_it->second.erase(temp_y_it);
            if (y_plus_it == temp_x_it->second.end()) {
              temp_y_it = y_minus_it;
            } else if (y_minus_it == temp_x_it->second.end()) {
              temp_y_it = y_plus_it;
...
deque<Node> MapSearchGreedy::getStartIndexList() {
  ...
  shuffle(start_index_list.begin(), start_index_list.end(),
